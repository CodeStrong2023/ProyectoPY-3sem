...
def obtener_empleado_correo(session, e_email): # Obtiene un registro de la bd que coincida con el correo dado
...
def verificar_empleado_registrado(session, correo, contrasenia): # Verifica los datos del empleado registrado
    empleado = obtener_empleado_correo(session, correo)
    if empleado and verificar_contrasenia(contrasenia.encode("utf-8"), empleado.contrasena):
        return empleado
    return None
...
def verificar_contrasenia(contrasenia, contrasenia_encriptada): # Verifica la contraseña encriptada
    resultado = bcrypt.checkpw(contrasenia, contrasenia_encriptada) # Compara la contraseña dada con la contraseña encriptada en la bd
    return resultado